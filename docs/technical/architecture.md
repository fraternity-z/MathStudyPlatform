...
资源中心 2026-09-18 增量：在既有统一授权上增加可解释授权结果与六类租户配置审计，原布尔权限谓词保持兼容；失败任务和 generation 运维读模型携带租户/知识库上下文并按同租户关联。解释函数只供受控 SQL 排障，不对普通 HTTP 开放；审计数据库身份不等同应用 actor。单站点仍按知识库与代际独占 collection，使用可信服务账号，尚未启用 RLS 或拆分数据库角色。详见[租户边界](vector-tenancy.md)、[授权隔离验收](../plans/resource-center-qdrant/TEST-P6-AUTH-2026-09-18.md)、[检索上下文](vector-advanced-retrieval.md)及[小范围部署](vector-small-deployment.md)。
...
P5 的 `0023` 为中文改写补充汉字二元词项 GIN 索引：正文和标题分别召回，以查询词项覆盖量参与词法排序，正文至少命中两个词项（短查询按实际词项数），标题命中可进入有限文档预览；英文 FTS 与完整中文子串继续有效。单一标题最多引入请求候选上限数量的已索引块，正文词项独立命中并保留标题/正文组合查询。词项函数版本不可原地改写，避免表达式索引漂移。所有候选仍只携带身份，并沿用发布/manifest 核验、RRF 和两次 PostgreSQL 授权。此能力改善词法重叠，不承诺无共同词项的语义同义改写；质量和负载证据见[中文召回验收](../plans/resource-center-qdrant/TEST-HAN-2026-09-08.md)。
...
Tutor 首次答疑、续聊及知识学习共用 `buildAssistantMessage`，通过可选的 `StudentContextReader` 逐轮读取认证学生本人的学习证据。生产装配由 `SessionRepository` 实现该只读端口，使用单条 PostgreSQL SELECT 保持语句级快照一致性，不依赖画像已生成，不增加数据库迁移或客户端学生 ID 参数。

- 当前问题优先、会话主题次优，按知识点中英文名称的字面子串匹配最多 6 个知识点；未命中时读取该生有限的薄弱知识点，标记为 `recent_learning`，只作为一般学习背景。
- 掌握度携带置信度、作答次数与最近作答时间，并复用 `masteryprojection.Current` 做只读遗忘投影。未观测或无效状态以 `estimated_mastery=null` 表示证据不足，不当作零掌握度。
- 错因从最近 30 天最新 100 次已提交错误作答中选择最多 3 条诊断样本；知识点依次取作答冻结快照、每日题快照和题库回退，不传题干、答案、诊断解释或建议正文。保留四类当前错因及历史 `calculation` 类型，样本不代表完整错误统计。
- 先修方向沿用学习路径的 `source=学习目标、target=前置知识` 契约，从最多 12 条候选筛选最多 4 条未达到掌握度 0.85 / 置信度 0.5 门槛的先修状态；缺少证据时先检查，不直接断言学生不会。
- 状态读取使用 500 ms 子超时，失败只记录固定降级原因并继续普通答疑，父请求取消仍向上传播。JSON 快照最多 4 KiB，携带 `as_of` 与证据说明，按完整条目裁剪，与资料、历史等共同遵守 16 KiB 动态输入预算。

快照作为独立资料消息传入 Tutor，固定规则要求依据当前主题和错因调整讲解、优先本轮状态、忽略 JSON 字符串中的指令，不逐项播报画像、不暴露内部 ID、不提前泄露练习答案。快照原文不写入会话消息；聊天或学生自我确认不更新 DKT，掌握度仍由正式作答链路更新。

先修提示依赖当前图谱数据正确性。初始化种子中存在关系端点与现有运行契约方向相反的历史数据，真实数据库的核对与条件修复在 [当前待办](../TODO.md) 跟踪；本次接入不更改已有关系或迁移。

## API 与兼容契约

- AI 辅导收敛为“问题答疑 / 知识学习 / 习题练习”三个入口，继续使用 `chat/study/practice` 标识。历史 `explain` 在界面显示为问题答疑，后端保留深入讲解偏好，新界面不再创建 explain。三个入口共享 Tutor，应用层组装公共模式规则与学习阶段上下文，适配层继续负责调用。答疑提供详细讲解、展开推导和举例快捷操作。
- 知识学习的 `POST /session/study` 使用客户端 UUID v4 原子、幂等创建会话与安排，主题保留最多 200 字，会话模块统一生成最多 36 字的标题。`GET/PATCH /session/{session_id}/study` 只允许所有者读取、活动 study 会话写入；next 携带 revision 并在会话行锁内重新校验。固定安排为基础回顾、概念理解、例题推导、理解检查、回顾总结。会话模块持久化回复关联和终态；学习模块独立记录当前 revision 的 `start/rephrase/reply/discuss` 操作，应用层统一计算 `can_advance/blocked_reason`，不解析消息文字。需本环节最新一轮有完整回复才能推进；理解检查还需最近 start 的完整出题及随后 reply 的完整反馈，重复出题、重新解释和中断不算作答。旧客户端未标记的对话记为 discuss，旧安排保留当前进度，但不猜测旧消息终态。客户端在聊天结束后刷新安排，请求结果不明时只读刷新，不自动重放 next。进度是学生自我确认，不表示模型认定掌握或产生测评成绩。保存安排后即进入会话列表，用户主动开始环节才调用模型。接口与协作边界见[开发指南](development.md)。
- 学习和练习会话跨类型切换通过新会话或既有练习页面完成，原进度保留；后端 mode API 对这类原地切换返回 `409 SESSION_MODE_LOCKED`。习题练习复用 `/exercise` 的选题、生成、幂等提交和提交后解析契约，不增加第二套判题或错题记录。学习主题可携带到练习页，存在精确匹配知识点时预选，否则请学生确认。练习页提示/相关知识在新标签页打开，当前题目与作答组件不卸载；提示页通过受保护的题目详情重新取题，不传标准答案。完整解析继续要求已有正式作答，本版不提供提交前查看解析，也不承诺聊天模型绝不自行推导答案。

- 消息中心的私信、通知和答疑列表统一按当前页刷新，删除或确认导致当前页超出总页数时回退到最后一页，不累积之前的页面。私信与答疑只有在页面可见、对应栏目打开且消息末尾完整进入视口时才推进已读位置，查找聊天记录时不推进；发送接口返回的持久化消息立即合入当前对话，后续列表刷新失败不撤销成功发送的展示。两端复用可见消息 Hook 和数学 Markdown 展示组件。私信历史搜索使用 `GET /api/v1/conversations/{id}/messages?search=...&page=1&page_size=50`，仅会话参与者可访问，不产生已读副作用；关键词按空白分词并按大小写不敏感的字面子串同时匹配正文，搜索文本上限 200 字，页码范围 1–10000、每页 1–100 条，结果先按最新消息分页、页内按时间正序展示。前端搜索按需请求单页并取消过期请求。消息摘要在服务端分页前优先返回待处理项目；论坛帖子列表支持 `scope=unread`，按收件人所有权和通知目标有效性筛选未读互动。以上变化不修改数据库结构，无需数据迁移。
...
- Session 首次、续聊、流式及历史重开统一传递 `knowledge` 模式、降级信息和实际送入 prompt 的引用。知识正文作为独立的不可信资料消息，固定 Tutor 规则禁止执行资料内指令。16 KiB 动态输入预算共同约束当前问题、模式、附件占用、历史、完整知识序列化与学生状态快照；知识最多 8 KiB、学生状态最多 4 KiB，均按完整条目保留，固定系统规则另占模型输入容量。数据库只保存引用元数据；带旧知识引用的助手回复不再次进入模型历史，以免撤权后重放资料。
...
- AI/Agent 通过 Eino 和 OpenAI-compatible provider 接入，运行配置持久化到数据库。Provider 的纯主机根地址会幂等补齐 `/v1`，带路径的地址视为完整 API base 并原样保留，以兼容 `/v1beta/openai` 等非 `/v1` 端点；Tutor 会话启用 Eino 上游流式输出，模型分片经 application callback 直接写入并刷新 SSE。流式与非流式请求均遵循渠道协议偏好：OpenAI 类型（含历史 `openai-responses`）优先使用 Responses，其他类型保留自动选择；推理模型（含 provider 命名空间下的 `gpt-5*`、`o1*`、`o3*`、`o4*`）省略采样参数并优先使用 Responses，仅在端点明确不支持时回退 Chat Completions。流式 Responses 在适配层逐片转换为 Eino 可消费的 Chat SSE，保留文本、拒绝、函数调用、完成原因和 usage；流式协议缓存与非流式隔离，收到成功 HTTP 响应后发生的流协议错误不再触发端点回退。另提供 Bearer 鉴权的 `POST /v1/responses` OpenAI 兼容入口：请求 `model` 是数据库中的逻辑模型名，调度器复用 provider 优先级、权重、密钥轮换、供应商模型映射、超时和候选重试；渠道原生支持 Responses 时透传 JSON/SSE，否则通过独立兼容层转换为 Chat Completions，并在已向客户端交付首个流事件后禁止切换渠道。默认模型客户端共享进程级安全 Transport 和连接池，单请求仍保留独立总超时，单 provider 主机最多保留 20 条空闲连接。Top P 已从管理端和运行时停用；管理端保存的建议值为 Temperature `1.0`、Max Tokens `4096` 和最大重试 `3` 次，但 Agent 只在显式覆盖时启用采样或应用层重试，Responses 兼容入口则使用模型级重试基线。模型请求总超时默认 `1800` 秒；Agent 迭代上限独立于网络重试次数。
...
PostgreSQL 是业务、版本和权限数据源，Redis 用于缓存和运行时辅助状态，Qdrant 仅保存可重建的向量和最小 payload。数据库结构由 `backend/migrations/` 中的 Go forward migration 管理；`0017` 追加资源中心契约，`0018` 追加管理员不可变模型配置，`0019` 增加 `pg_trgm`/检索索引与 nullable 会话引用元数据，`0020` 追加入库幂等、任务关联与对账游标，`0021` 追加终态保留快照与未引用上传 staging。`0022` 增加发布审批门与运维事务审计，`0023` 增加中文二元词项索引，`0024` 保存词法生成列与索引，`0025` 增加租户成员、部门、统一资源 ACL、复合租户外键和资源限额，`0026` 增加知识学习进度，`0027` 增加回复终态、学习操作序列和安排创建幂等标识，`0028` 增加可解释资源授权与租户配置审计，后续从 `0029` 起追加。历史 Alembic 链和开发期增量链已退出当前工作区。迁移规则见 [Go 数据库迁移策略](../../backend/migrations/README.md)。
