...
`deploy/vector/compose.yaml` 是隔离的三 peer 准生产演练入口。同一 Docker 宿主机没有宿主机/可用区容灾能力。2026-09-16 项目负责人明确只小范围上线、不建设跨区域能力，当前采用[单站点部署手册](vector-small-deployment.md)，不要求为本项目增加三个独立故障域。若未来需要此类 HA，再使用独立持久卷、私网 DNS 和受控负载均衡重新验收。历史演练每节点初始 2 CPU/2 GiB，存储至少保留 20% 空闲及一次完整蓝绿代、快照所需空间。
