...
后台 AI provider 的 `base_url` 可以填写纯主机根地址或完整 API base；纯主机地址会自动补 `/v1`，只要地址中已有路径就会原样使用，因此 `/v1`、`/proxy/v1`、`/v1beta/openai` 均不会被重复改写。OpenAI 类型（含历史 `openai-responses`）的连接测试及非流式调用默认优先使用 Responses，仅在端点明确不支持时回退 Chat Completions；其他类型保留自动兼容策略。推理模型按大小写不敏感的 `gpt-5*`、`o1*`、`o3*`、`o4*` 前缀识别，也兼容 `provider/model` 命名空间，并优先尝试 Responses。非流式 Responses 请求显式发送 `stream:false` 和 `Accept: application/json`；若兼容上游仍返回 SSE，则提取终止事件的完整响应，失败或缺少终止事件时报协议错误。连接测试对推理模型使用 `max_completion_tokens=32`，对旧式 Chat provider 保留 `max_tokens=32`。
