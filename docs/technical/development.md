...
升级代码后先排空并停止旧 API，执行迁移至 `0027_session_study_turns`，再启动新版后端和前端。问题答疑应直接回应具体疑问，并能通过快捷操作深入讲解；历史 explain 会话显示为问题答疑并保留消息。知识学习填写主题和基础后建立五步安排，主动开始当前环节，确认理解后推进；刷新与重新打开应恢复进度。验证重复点击只有一个 revision 推进、越权返回 404、未完成对话/理解检查或中断输出不能推进，跨类型新建会话保留原记录。
...
当前迁移链是 `0001` 至 `0027`。`0017`–`0024` 建立资源数据、模型、异步任务、发布审计和词法性能契约；`0025` 新增租户成员/部门、统一资源授权、复合租户外键及文档/字节/任务额度；`0026` 增加知识学习进度，`0027` 增加回复终态、学习操作序列和安排创建幂等标识。空库应记录 27 个版本，version 25 库只新增 26 和 27，复跑无待应用版本。迁移账户须可在 `public` 安装 `pg_trgm` 并创建索引；先停止 API/worker 写入、备份、迁移，再启动新进程。`0024` 会重写表，`0025` 会核验已有数据的租户关联，`0026` 和 `0027` 会增加学习流程结构，均需维护窗口。历史草稿按[迁移策略](../../backend/migrations/README.md)校准，不得删除账本重放；后续从 `0028` 起追加。
...
消息中心与微信公众号基础分别由 `0003`、`0004` 交付；运行前应用当前全部 `0001` 至 `0027` 迁移。空库首次记录 27 个版本，已有库只新增尚未应用的版本，第二次运行无待应用版本。
