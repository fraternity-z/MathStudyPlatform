...
当前迁移链是 `0001` 至 `0024`。`0017` 至 `0021` 建立资源中心数据、模型、检索、异步任务和保留清理契约；`0022` 增加发布审批和审计，`0023` 增加中文词项，`0024` 保存词法生成列及索引。空库首次记录 24 个版本，version 23 库只新增 24，复跑无待应用版本。迁移账户须可在 `public` 安装 `pg_trgm` 并创建索引；先停止 API/worker 写入、备份、迁移，再启动新进程。`0024` 会回填并重写表，需预留维护窗口和磁盘。历史草稿按[迁移策略](../../backend/migrations/README.md)校准，不能删除账本重放；后续从 `0025` 起追加。
...
消息中心与微信公众号基础分别由 `0003`、`0004` 交付；运行前应用当前全部 `0001` 至 `0024` 迁移。空库首次记录 24 个版本，已有库只新增尚未应用的版本，第二次运行无待应用版本。
