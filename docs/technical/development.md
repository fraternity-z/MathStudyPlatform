...
升级代码后先执行 `0024_session_study_progress` 迁移，再启动新版后端。问题答疑应直接回应具体疑问，并能通过快捷操作深入讲解；历史 explain 会话显示为问题答疑并保留消息。知识学习填写主题和基础后建立五步安排，主动开始当前环节，确认理解后推进；刷新与重新打开应恢复进度。验证重复点击只有一个 revision 推进、越权返回 404、未完成对话/理解检查或中断输出不能推进，跨类型新建会话保留原记录。
