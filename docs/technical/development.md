...
升级代码后先排空并停止旧 API，执行迁移至 `0025_session_study_turns`，再启动新版后端和前端。问题答疑应直接回应具体疑问，并能通过快捷操作深入讲解；历史 explain 会话显示为问题答疑并保留消息。知识学习填写主题和基础后建立五步安排，主动开始当前环节，确认理解后推进；刷新与重新打开应恢复进度。验证重复点击只有一个 revision 推进、越权返回 404、未完成对话/理解检查或中断输出不能推进，跨类型新建会话保留原记录。
...
当前迁移链是 `0001` 至 `0024`。`0017` 至 `0021` 建立资源中心数据、模型、检索、异步任务和保留清理契约；`0022` 增加 generation 发布审批门和事务运维审计，`0023` 增加中文词法索引，`0024` 增加知识学习进度。空库首次记录 24 个版本，version 23 库只新增 24，复跑无待应用版本。迁移账户须可在 `public` 安装 `pg_trgm` 并创建索引；先停止 API/worker 写入、备份、迁移，再启动新进程。历史草稿按 [迁移策略](../../backend/migrations/README.md) 校准，不能删除账本重放；后续从 `0025` 起追加。
