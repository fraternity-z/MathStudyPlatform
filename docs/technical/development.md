...
升级代码后先排空并停止旧 API，执行当前全部迁移至 `0028_resource_access_decisions_and_tenancy_audit`，再启动新版后端和前端。问题答疑应直接回应具体疑问，并能通过快捷操作深入讲解；历史 explain 会话显示为问题答疑并保留消息。知识学习填写主题和基础后建立五步安排，主动开始当前环节，确认理解后推进；刷新与重新打开应恢复进度。验证重复点击只有一个 revision 推进、越权返回 404、未完成对话/理解检查或中断输出不能推进，跨类型新建会话保留原记录。

学习模块接口契约（路径省略 `/api/v1`）：

| 接口 | 请求 | 结果与责任 |
| --- | --- | --- |
| `POST /session/study` | `session_id`（客户端 UUID v4）、`topic`（1–200 个 Unicode 字符）、`foundation` | 同一事务建立会话、欢迎消息和学习安排；返回 `session_id/progress`。同一 ID 和规范化载荷重试返回已有安排，载荷或所有者不同返回 `409 SESSION_ID_CONFLICT`。标题由后端截取前 36 字，主题完整保留。 |
| `GET /session/{id}/study` | 无 | 返回安排或 `null`；包含 `can_advance`、`blocked_reason`。原因码为 `reply_required`、`answer_required`、`completed`，允许推进时为空串。 |
| `PATCH /session/{id}/study` | `action=start` 与主题/基础，或 `action=next` 与 `revision` | start 用于已有 study 会话；next 在会话行锁内重新校验相同推进规则和 revision，冲突返回 `409 STUDY_PROGRESS_CONFLICT`。 |
| `POST /session/{id}/chat` | 既有聊天字段，可选 `study:{revision,action}` | action 为 `start/rephrase/reply`，分别表示开始、换种讲法、输入回应；版本过期在写用户消息前返回 409。无结构化字段的旧客户端正常聊天，但不能作为理解检查的作答证据。 |

会话模块保存 `reply_to/completion_status`；学习模块记录当前 revision 的操作序列。推进条件在应用层统一定义，前端不解析按钮文字或中断尾注。理解检查需要最近一次 start 的完整导师出题，以及之后最新 reply 的完整反馈；再次 start 或 rephrase 都不能冒充作答。没有配置 Tutor 的提示也不计为完整教学回复。历史安排保留当前 step/revision，不根据旧消息猜测终态；未完成的当前环节需要重新开始对话。其他创建会话接口的标题超过 36 字返回 422。

前端协作边界：`modules/exercise/tutorContext.ts` 只构造题目辅导上下文，`useExerciseTutorContext` 只负责读取、取消、过期响应隔离与重试，不依赖 Redux、Toast 或路由；页面负责消费结果并导航。学习面板调用学习接口、展示后端推进状态；每次聊天结束后重新读取安排，发送操作时携带已读取的 revision。创建超时保留当前表单的稳定请求 ID，显式重试；推进失败先读取结果，不自动重放 next。

回归至少覆盖 36/37/200 字及表情主题、超过 200 字拒绝、重复/并发创建、事务回滚、旧会话恢复、未开始/停止/中断/重复出题/换种讲法不可替代作答、完整反馈后推进、过期版本及并发 next；浏览器交互覆盖题目慢加载时移除查询参数、旧响应晚到、失败重试、StrictMode 和创建超时重试。

习题练习使用原有练习页面，确认知识点与难度后出题。提示及相关学习使用新标签页，原题与未提交输入保留；正式提交沿用原幂等 key、判题、错题与掌握度更新，完整解析仍只在提交后开放。学习页携带的自由文本主题不是知识点 ID，没有精确匹配时不得自动冒充已匹配。学习进度不计为测评成绩。
...
当前迁移链是 `0001` 至 `0028`。`0017`–`0024` 建立资源数据、模型、异步任务、发布审计和词法性能契约；`0025` 新增租户成员/部门、统一资源授权、复合租户外键及文档/字节/任务额度；`0026`、`0027` 增加知识学习进度和结构化操作证据；`0028` 增加可解释资源授权和六类租户配置变更审计。空库应记录 28 个版本，version 27 库只新增 28，复跑无待应用版本。迁移账户须可在 `public` 安装 `pg_trgm` 并创建索引；先停止 API/worker 写入、备份、迁移，再启动新进程。`0024` 会重写表，`0025` 会核验已有数据的租户关联，`0026` 至 `0028` 会增加学习或资源隔离结构，均需维护窗口。历史草稿按[迁移策略](../../backend/migrations/README.md)校准，不得删除账本重放；后续从 `0029` 起追加。

## 环境配置

P5 新增 `QDRANT_SEARCH_HNSW_EF`（0–8192，默认 0），仅由部署环境控制近邻搜索预算；应先在目标数据/过滤条件下评测再提高，不能由公开搜索请求指定。正文重复候选在当前 manifest 筛选与授权后去重；召回后仍保留最终 SQL 预算并再次鉴权。并发评估、恢复和参数回滚见[运行手册](vector-operations.md)及[质量评估](vector-quality-evaluation.md)。
...
后台 AI provider 的 `base_url` 可以填写纯主机根地址或完整 API base；纯主机地址会自动补 `/v1`，只要地址中已有路径就会原样使用，因此 `/v1`、`/proxy/v1`、`/v1beta/openai` 均不会被重复改写。OpenAI 类型（含历史 `openai-responses`）的连接测试、非流式及 Tutor 流式调用默认优先使用 Responses，仅在端点明确不支持时回退 Chat Completions；其他类型保留自动兼容策略。推理模型按大小写不敏感的 `gpt-5*`、`o1*`、`o3*`、`o4*` 前缀识别，也兼容 `provider/model` 命名空间，并优先尝试 Responses。非流式 Responses 请求显式发送 `stream:false` 和 `Accept: application/json`；若兼容上游仍返回 SSE，则提取终止事件的完整响应，失败或缺少终止事件时报协议错误。流式请求发送 `stream:true` 和 `Accept: text/event-stream`，不携带 Chat 专属的 `stream_options`。协议成功缓存区分流式与非流式，避免连接测试影响 Tutor 的端点选择。连接测试对推理模型使用 `max_completion_tokens=32`，对旧式 Chat provider 保留 `max_tokens=32`。

管理端在渠道或智能体配置完整保存后显示成功反馈，智能体配置保存成功后自动收起当前配置框；保存失败时保留当前表单并显示服务端原因。渠道编辑只有在渠道信息和模型列表都更新成功后才算完成。“已保存”仅表示配置已持久化，“已配置”还表示智能体存在启用的候选渠道，但两者都不代表外部模型可调用；真实连通性仍以渠道的“测试连接”结果为准。连接测试校验生成结果而非仅检查 HTTP 2xx。模型能力 `embedding` / `rerank` 的显式布尔配置优先；缺少配置时按导入模型 ID 中的 `embed` / `rerank` 名称提示选择测试用途。嵌入模型请求 `/v1/embeddings` 并验证非空向量；重排模型明确提示通用测试尚不支持 `/rerank`，不调用生成端点。
...
学习会话的 `POST /session/start-chat` 和 `POST /session/{session_id}/chat` 使用真正的模型分片流，而不是等待完整回复后再包装为 SSE。事件顺序为可选的 `session_info`、一次 `task_info`、多次 `message` chunk、一次 `message` done；每个事件写入后立即 flush。前端以 `session_info` 或 `task_info` 确认服务端接收，缺失这两类元信息时以首个 chunk 或 done 兜底确认；确认后的轮次不会因断流恢复成待发送内容，也不自动重放。Tutor 根据渠道及模型选择 Responses 或 Chat Completions；Responses 适配器通过带背压的管道逐片交付文本、拒绝及函数调用，不等待完整结果。输出项快照与终态仅补齐尚未交付的后缀，校验已交付内容和响应元数据一致，并转换完成原因及 usage；拒绝文本同时进入 Eino 的 content 以便正常显示。只有有效的 `response.completed` / `response.incomplete` 才生成 Chat `[DONE]`，失败、取消、无终态 EOF 或格式错误均作为读取错误上抛，沿用应用层部分回复保存机制；关闭流或取消上下文会关闭上游响应体。成功 HTTP 响应后的流错误不触发协议回退，已输出内容后仍遵守既有候选模型禁止切换规则。事件契约参照 [OpenAI 官方流式文档](https://developers.openai.com/api/docs/guides/streaming-responses)。默认模型请求共享进程级 HTTP Transport 和连接池，但通过客户端浅拷贝保留每次运行配置的独立总超时；显式注入的测试客户端仍按请求单独包装。
...
消息中心与微信公众号基础分别由 `0003`、`0004` 交付；运行前应用当前全部 `0001` 至 `0028` 迁移。空库首次记录 28 个版本，已有库只新增尚未应用的版本，第二次运行无待应用版本。
