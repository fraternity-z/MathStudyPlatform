...
当前迁移链是 `0001` 至 `0025`。`0017`–`0024` 建立资源数据、模型、异步任务、发布审计和词法性能契约；`0025` 新增租户成员/部门、统一资源授权、复合租户外键及文档/字节/任务额度。空库应记录 25 个版本，version 24 库只新增 25，复跑无待应用版本。迁移账户须可在 `public` 安装 `pg_trgm` 并创建索引；先停止 API/worker 写入、备份、迁移，再启动新进程。`0024` 会重写表，`0025` 会核验已有数据的租户关联，均需维护窗口。历史草稿按[迁移策略](../../backend/migrations/README.md)校准，不得删除账本重放；后续从 `0026` 起追加。
...
消息中心与微信公众号基础分别由 `0003`、`0004` 交付；运行前应用当前全部 `0001` 至 `0025` 迁移。空库首次记录 25 个版本，已有库只新增尚未应用的版本，第二次运行无待应用版本。
