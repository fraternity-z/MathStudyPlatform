...
> 专项状态：`IN_PROGRESS`
> 当前阶段：P5 `DONE`（11/11，本地原创基线；清理残留已获准作为例外）；P0-P4 已完成，P6 未启动，外部上线另行安排
> 当前里程碑：M5 `DONE`（本地原创合成数据的性能/质量验收）；P5 收尾完成
> 最后更新：2026-09-08
> 维护入口：[目录说明](README.md)
...
2026-09-08 当前执行：P5 的 11 项本地技术交付和 M5 门禁通过。自编 60 份四格式短讲义、240 正例、真实模型与 Tutor、十万条 ANN/副本对照、发布回滚/恢复完成；最终正式 HTTPS P95 947.896 ms，突发 914.276 ms，十分钟 3000 请求/7500 引用无错误或降级、P95 824.816 ms。已删除 20 份测试源码并停止/删除隔离服务；临时 dump/snapshot 等物理删除受限，仅占用本地磁盘；项目负责人已接受该残留作为收尾例外，P5 标记 DONE，残留不纳入交付。证据见[本轮 P5 验收](TEST-P5-2026-09-08.md)。下述停止记录为历史，不是当前执行门。

2026-09-08 最新执行指令：“完成这一阶段中断目标并更新文档，不要继续往下推了”。已收尾当前 P5 验证并停止新增工作。中文词法修复在新编 30 条改写中由旧谓词 0/30 提高到 Recall@5 1.0；真实 Qdrant 完成六组一万条、八组十万条索引对比，十万条 HNSW 数值 ANN Recall@10 为 0.541，质量门禁失败，不发布新参数。文本 42 子测试与入库 101 子场景通过 race，局部测试不代替真实模型/混合负载/soak。P5-02 完成，M5 仍未通过；P6 不启动。详见[索引实验与收尾](TEST-INDEX-2026-09-08.md)。

2026-09-08 后续：项目负责人授权自行编写测试材料，已完成 24 篇原创讲义、125 个冻结检索场景及 288 次引用核验，真实 PG/正式 HTTP/检索应用与 Mock 向量依赖通过 race 验证。另完成授权撤销竞态、152 个切块场景、26 个错误边界及 5 并发/250 请求。中文自然改写问句的 FTS Recall@5 为 0/24，保留为待优化项，不以精确词条得分掩盖；不再以真实教学语料缺失阻断后续合成测试。见[合成验收](TEST-SYNTHETIC-2026-09-08.md)，M5 未通过。

2026-09-08 启动 P5 工具交付：项目负责人确认真实教学语料尚未准备，本轮先推进工具与 Mock 验证。新增持续运维质量评估器，冻结输入 checksum、正常/降级分组排名门槛、引用/授权/空结果检查、脱敏原子报告和明确退出码；9 个临时测试方法及子场景通过，标准库 trace 可执行行覆盖 99%。真实语料、容量、Tutor 无答案和参数实验尚未验收，P5-01/P5-09 为 `IN_PROGRESS`，不增加已完成任务计数。使用与证据见[评估手册](../../technical/vector-quality-evaluation.md)。

2026-09-07 完成 P4 本地准生产交付：三个非 root TLS Qdrant peer、双 worker、发布审批/回滚/失败任务审计、指标告警、加密备份恢复与丢失 collection 重建。真实 Prometheus/Alertmanager 发出故障及恢复回执；修复镜像安全项并验证滚动。项目负责人明确“先完成本地准生产演练，外部上线另行安排”。详细口径见 [P4 验收](TEST-ACCEPTANCE-2026-09-07.md)，不把本地结果外推为生产规模或跨可用区承诺。

P0-P3/M3 已完成，当前 MVP 范围无未完成开发或验收项。PDF/DOCX/TXT/MD 异步入库、幂等任务、租约、发布/撤回/删除、重建/对账和保留清理均已实现并通过测试。60 篇原创讲义使用管理员真实模型生成向量；最终 100 条冻结查询的 Recall@5、MRR@5、nDCG@5 均为 1.0，500 次引用身份及原文校验通过。修复 PostgreSQL 扫描问题后，同版本 Linux Qdrant 的 10 万条/5 并发本地与完整链路 P95 为 427/622 ms，达到冻结门槛；百万档完成趋势评测，存在 4%-5% 降级。项目负责人明确授权后，5 次真实 Tutor 无答案问答全部成功，逐条核对无错误资料引用或来源冒认，M3 通过。
...
| 已完成任务 | 81 |
| 总体进度 | 87.1% |
| 待后续决策 | 0（本轮范围；外部上线环境和组织接入另行安排） |
| 当前范围开放高/严重风险 | 0（P0-P3 已实施缓解并验证） |
| 当前执行门 | P0-P5/M5 在各自批准范围通过；P5 清理残留已获准作为例外，P6 未启动 |
...
| [P3 检索与 RAG 集成](03-retrieval-and-rag-integration.md) | `DONE` | 13/13 | P2 已完成；固定质量/容量门槛通过 | M3 | 真实质量、引用、容量和真实 Tutor 无答案问答通过 |
| [P4 生产就绪](04-production-readiness.md) | `DONE` | 12/12 | P3 | M4 | 本地准生产安全、观测、恢复、故障和拓扑通过；外部上线另行安排 |
| [P5 性能与质量](05-performance-and-quality.md) | `DONE`（本地原创基线） | 11/11 | P4 | M5 | 原创质量、十万条容量/soak、发布恢复通过；清理残留按批准例外登记 |
...
| M3 MVP 可用 | `DONE` | P2-B、真实质量、引用、固定容量及 5 次真实 Tutor 无答案问答全部通过 |
| M4 生产就绪 | `DONE`（本地准生产） | 三节点 TLS、安全审计、真实本地告警、备份恢复、重建和故障演练通过；正式上线独立验收 |
| M5 性能质量达标 | `DONE`（本地原创基线） | 固定数据质量、十万条/五并发、混合读写和持续运行达到门槛；不代表外部生产或真实学期分布 |
...
| D-009 | PostgreSQL、对象存储和 Qdrant 的 RPO/RTO、备份及跨区要求 | `DECIDED`（本地范围） | 项目负责人确认范围 / Codex | 2026-09-07 | 本地演练目标 RPO 24h/RTO 60m；PG+对象导出+逐节点快照、age 加密、30 天保留策略。小数据恢复重建通过，不外推生产规模；正式调度、云对象版本/跨区另行上线验收 |
| D-010 | 单节点到 Qdrant cluster/Cloud 的切换阈值和生产拓扑 | `DECIDED`（本地范围） | 项目负责人确认范围 / Codex | 2026-09-07 | 本地三 TLS peer，3 shard/3 replica/write consistency 2、双 worker；固定 1.14.1 引擎与更新精简运行镜像。正式故障域/Cloud 采购与外部上线另行安排 |
...
| R-007 | 代表性数据不足导致索引和参数结论失真 | 中 | `MITIGATED`（本地原创基线） | 原创语义与容量副本/数值 ANN 分开计量；真实质量、十万条精度和完整负载通过，不外推真实学期分布或外部生产环境 |
...
| 备份恢复与 RPO/RTO | `DONE`（P4 本地） | PG/本地对象/节点快照、加密校验、丢失 collection 重建、容器故障已验证；生产规模、云对象和外部上线另验 |
| MVP 检索质量与目标容量 | `DONE` | 最终真实正例质量及 500 次引用核验通过；10 万条/5 并发本地及完整链路 P95 为 427/622 ms，均达到冻结门槛 |
| MVP 真实无答案问答 | `DONE` | 明确授权后完成 5 次真实 Tutor 调用；使用正式 Session 上下文路径和内存候选/存储替身，逐条答案核对无错误引用，25 条候选身份/hash 通过；不冒充新一轮 PG/Qdrant 集成或拒答率 |

## 7. 验证证据日志

2026-09-08 P5 最终：60 份原创四格式材料、240 正例、真实模型/Tutor、HNSW/副本、十万块恢复、五并发和十分钟稳定性验收通过。P5/M5 本地完成，11/11；20 份测试源码已删除，独立服务已清理，临时 dump/snapshot 等物理删除受限。项目负责人已接受不影响功能和验收的残留作为收尾例外，残留不纳入交付。详见[本轮验收](TEST-P5-2026-09-08.md)。

2026-09-07 P4：三节点/双 worker、TLS与文件密钥、22迁移、发布审批/回滚/空库边界、加密备份恢复/重建、真实故障与 Prometheus→Alertmanager 本地回执、容量 smoke 和安全镜像滚动均已验证。工程检查、覆盖率及安全扫描口径见 [P4 验收](TEST-ACCEPTANCE-2026-09-07.md)。

| 日期 | 阶段/任务 | 环境 | 命令或演练 | 结果 | 证据位置 |
|---|---|---|---|---|---|
| 2026-09-08 | P5-02 索引对比及当前验证收尾 | 独立 Qdrant 1.14.1、PG 18、Go Mock | 六组一万条/八组十万条索引实验；中文、文本和入库临时测试；race/build | 索引比较完成，十万条 ANN 基线 0.541 未达 0.90；其他局部契约通过；保留参数并按用户指令停止，不进入 P6 | [索引收尾](TEST-INDEX-2026-09-08.md)、[中文](TEST-HAN-2026-09-08.md)、[文本](TEST-TEXT-TUNING-2026-09-08.md)、[入库](TEST-INGESTION-TUNING-2026-09-08.md) |
| 2026-09-08 | P5 原创合成集成、切块与边界 | 独立 PG 18、正式 HTTPS/应用、Mock 向量与模型 | 全部 22 迁移；临时 Go race/coverage；正式 Python evaluator；后端 build | 125 场景/288 引用、授权竞态、152 切块、26 错误边界及 250 并发请求通过；FTS 改写 0/24 单列待调优；无真实模型费用 | [合成验收](TEST-SYNTHETIC-2026-09-08.md) |
| 2026-09-08 | P5-01/P5-09 工具切片 | 本地 Python 3.10、外部 HTTP Mock | 临时 unittest + 标准库 trace；CLI help；后端全量 test/build | 9 个测试方法及子场景通过，评估器可执行行覆盖 99%；Go 命令通过（无持久测试源码，不代替功能复测）；未调用真实模型或进行容量实验 | `../../technical/vector-quality-evaluation.md` |
...
| 2026-09-06 | 真实 Tutor 无答案验收 | 管理员模型、正式 Session/Agent、内存候选与存储替身 | 用户明确授权；冻结 5 问题/5 段无关材料；最多 5 次真实请求、只读配置；逐条完整回答与候选身份/hash 核验 | 5/5 成功，无错误资料引用；上下文每条 2908 字节，25 次身份/hash 核验通过。真实模型与测试替身边界单独记录，M3/P3 标记 DONE | `TEST-ACCEPTANCE-2026-09-06.md` 无答案问答门禁 |

不在此表粘贴密钥、Token、密码、真实 DSN 或受保护业务数据。长日志保存到受控构建系统，表中只保留摘要和链接。

## 8. 近期行动

清理授权及收尾例外均已获得。单个 snapshot 路径删除被拒绝的事实保留；测试 PostgreSQL 已停止，残留不参与应用运行，仅占用本地磁盘。按项目负责人“如果不影响的话直接记录完成即可”的决定，P5 收尾完成，不再以物理清理限制阻断阶段。

1. P4 已按批准的本地准生产范围完成。正式部署时落实真实 secret/证书、故障域、最小权限账号、云对象备份调度和组织告警接入，按运行手册验收；不重复计作本轮未完成功能。
2. P5 已完成，测试残留按批准例外保留在忽略目录，不纳入交付；执行环境允许时可另行物理清理。P6 未启动。

## 9. 更新记录

2026-09-08 最终收尾：新增 `0024`、词法/融合去重、召回预算、HNSW ef 配置、恢复统计和导入导出兼容；P5/M5 本地完成，81/93 项。项目负责人已接受不影响功能和验收的测试残留作为清理例外，P5 标记 DONE，P6 未启动。

2026-09-08：中文召回修复与 P5 索引/文本/入库验证收尾；仅 P5-02 增计完成，合计 71/93。十万条数值 ANN 质量失败已记录，M5 未通过；用户明确停止后续推进，P6 未启动。测试容器/collection 和源码已清理，自动审批拒绝删除的旧临时目录单独登记，不把整个专项标为完成。

2026-09-08：按“没有真实语料，你直接自己编写测试即可”补齐原创合成检索、真实 PG/HTTP、切块与错误场景、权限竞态及小型并发验证；P5-01/06/09/10 进行中，总计仍为 70/93。记录 FTS 中文改写零召回，M5 未通过，后续测试继续自行构造材料。

2026-09-08：按项目负责人“尚未准备，先推进工具与验证”的回复交付 P5 质量评估工具及 Mock 验证，P5 改为 `IN_PROGRESS`；未将检索空结果等同 Tutor 拒答，未将顺序采样耗时等同容量验收。总计仍为 70/93，M5 未通过。

2026-09-07：按“先完成本地准生产演练，外部上线另行安排”关闭 P4/M4 本轮范围，P4 12/12，总计 70/93；D-009/D-010 本地决策和运行手册落地，外部上线不冒充已完成，下一开发阶段为 P5。

| 日期 | 变更 |
|---|---|
| 2026-09-06 | 项目负责人明确允许 5 次真实 Tutor 调用，全部成功且无错误资料引用；补齐候选身份/hash 和完整答案人工核验，M3/P3 标记 DONE。总任务仍为 58/93，前次已计入 P3 的 13 项开发，本次不重复计数。 |
