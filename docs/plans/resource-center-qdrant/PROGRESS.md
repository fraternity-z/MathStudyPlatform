...
> 专项状态：`IN_PROGRESS`
> 当前阶段：P6 `IN_PROGRESS`，小范围核心交付已通过；P0-P5 已完成，原 P6 高级项尚未全部关闭
...
| 当前执行门 | P6 授权隔离切片验证通过；RLS、角色拆分和其他原高级项未全部关闭 |
...
| M6 高级能力就绪 | `IN_PROGRESS` | 单站点核心已通过；原高级项未全部实现，不以跨区前置阻断本地开发 |
...
2. P5 已完成，测试残留按批准例外保留在忽略目录，不纳入交付。P6 授权解释、白名单审计和运维同租户关联切片已通过验证；继续完成 RLS/数据库角色拆分及 P6 页列出的其余高级项。
