...
> 专项状态：`IN_PROGRESS`
> 当前阶段：P6 `IN_PROGRESS`，本轮小范围核心回归通过（真实 PG/Qdrant、Mock embedding）；P0-P5 已完成，原 P6 高级项暂缓
> 当前里程碑：M5 `DONE`；M6 保留 `IN_PROGRESS`，不把本轮核心等同原全部高级能力
> 最后更新：2026-09-18
> 维护入口：[目录说明](README.md)
...
2026-09-18 当前交付：小范围核心回归通过。全新 PostgreSQL 18 完成 28 个迁移及重复执行；单节点 Qdrant 1.14.1 完成 TXT 上传、worker 索引写入、hybrid 检索、引用、撤回和 purge，错误维度拒绝及 Qdrant 不可达降级通过，后端构建通过。embedding 与认证主体使用 Mock，本地对象存储使用正式实现，不代表真实模型质量、完整登录或容量复测。Windows 深路径问题通过短数据目录解决；停止服务后的物理清理受限已记录。详见[小范围验收](TEST-SMALL-2026-09-18.md)。原 P6 计数不变，RLS、角色拆分、多集群、分布式扩展、弹性伸缩及其他高级项暂缓。

2026-09-18：P6 授权隔离切片继续推进。`0028` 增加可解释知识库/资源授权、保留原布尔谓词兼容，并对白名单内六类租户配置变更进行事务审计；失败任务和 generation 运维查询补齐 tenant/knowledge base 上下文及同租户重试关联。该切片验证证据见[授权隔离验收](TEST-P6-AUTH-2026-09-18.md)。解释函数仅用于受控 SQL，不对普通 HTTP 开放；审计数据库身份不是应用 actor。服务账号仍处于可信边界，RLS 与数据库角色拆分尚未完成，因此 P6-02/03/06 只记部分进展，计数保持 82/93、P6 1/12，M6 保持 `IN_PROGRESS`。

2026-09-16：P5 已提交 `41aa6b9`。用户明确小范围上线、不需要跨区域，P6-07/08 移出本轮。完成 `0025` 租户成员/部门、统一授权、附件隔离、额度和上下文去冗余；真实 PG、独立 Qdrant、HTTP/Mock/race 与工程检查通过。模型维护切换沿用管理员与对账保护。多模态、跨语言质量、自动查询扩展和 shadow/双写尚未交付，剩余范围待收敛，M6 不标 DONE。详见[本轮交付](TEST-P6-2026-09-16.md)。下述任务计数按原 93 项保留追溯，不把移出项计为实现完成。
...
|---|---:|
| 总任务 | 93 |
| 已完成任务 | 82（原清单口径；新增 P6-01） |
| 总体进度 | 88.2%（原清单；2 项按小范围部署移出，不计完成） |
| 待后续决策 | 0（本轮范围；外部上线环境和组织接入另行安排） |
| 当前范围开放高/严重风险 | 0（P0-P3 已实施缓解并验证） |
| 当前执行门 | 小范围核心回归通过；本轮收尾，原高级项暂缓 |
...
| [P6 多租户、高可用与智能增强](06-multitenancy-ha-and-intelligence.md) | `IN_PROGRESS` | 1/12（原清单；其余含部分交付及 2 项移出） | P5 | M6 | 单站点核心已交付，原高级项范围待收敛 |
...
| M6 高级能力就绪 | `IN_PROGRESS` | 本轮只验收小范围核心流程；原高级项暂缓且不计完成 |
...
| R-006 | 默认租户模型被误宣称为完整多租户隔离 | 严重 | `MITIGATED` | 租户领域、组合 ACL、可解释授权和白名单审计已实现部分控制；可信服务账号尚无 RLS/角色拆分，placement 与完整多租户验收仍属于 P6 |
...
|---|---|---|---|---|---|
| 2026-09-18 | P6-02/03/06 授权隔离切片 | 隔离 PostgreSQL 18、后端本地验证 | `0028` 空库/升级迁移、授权原因码与兼容谓词、六类审计、运维同租户关联及工程检查 | 本轮切片通过；RLS、数据库角色拆分与 P6 完整矩阵未完成，不增加任务计数 | [授权隔离验收](TEST-P6-AUTH-2026-09-18.md) |
...
1. 本轮小范围核心回归已完成，实际结果与 Mock 边界见[验收记录](TEST-SMALL-2026-09-18.md)；隔离服务已停止，清理受限残留单独登记。
2. 保持 82/93、P6 1/12 和 M6 `IN_PROGRESS`，不以核心回归虚增原高级能力计数。正式环境的模型、身份认证与部署验收另行执行。
3. RLS、数据库角色拆分、多集群、分布式扩展、弹性伸缩、多模态、跨语言质量、自动查询扩展及 shadow/双写均暂缓，后续由明确需求重新排期。

## 9. 更新记录

2026-09-18：按“小范围测试、尽快完成”完成数据库、向量写入、索引、相似度检索与基础异常回归，详见[验收记录](TEST-SMALL-2026-09-18.md)。原 P6 计数不变，高级隔离与扩展项暂缓。

2026-09-18：完成 `0028` 授权解释与租户配置审计切片，并补齐失败任务/generation 的租户运维上下文；本轮验证通过。使用边界限定为受控 SQL、白名单审计和可信服务账号，RLS/角色拆分仍未完成。P6-02/03/06 保持部分进展，82/93、P6 1/12 与 M6 `IN_PROGRESS` 均不变。

2026-09-16：P6-01 完成；交付统一租户权限、原文附件 deny、知识库幂等、额度/重试与上下文去冗余，终审两项缺陷已修复并真实 PG 复现验证。跨区域建设按用户要求移出，本轮实际能力不外推 RLS、多模态和无损跨模型切换。独立 PG/Qdrant 已停止，临时源码已删除，Qdrant 目录清理拒绝如实记录。
